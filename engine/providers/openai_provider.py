...
class OpenAIProvider(BaseProvider):
    ...
    def _initialize_client(self, fallback_key: Optional[str] = None):
        ...
        if not key:
            ...
            logging.getLogger(__name__).warning(
                "OPENAI_API_KEY not available yet (keys may be in cooldown)."
            )
    ...
    def _ensure_client(self):
        ...
        if self._raw_client is None:
            raise RuntimeError("OpenAI provider has no active API key.")
    ...
    async def generate_response(
        ...
        for attempt in range(max_retries):
            try:
                ...
                if (
                    "429" in error_str or "rate limit" in error_str or "403" in error_str
                ) and self.key_pool:
                    is_rate_limit = "429" in error_str or "rate limit" in error_str
                    self.key_pool.mark_exhausted(
                        "openai", self.current_key, is_rate_limit=is_rate_limit
                    )
                    self._initialize_client()
                else:
                    raise e
    ...
    async def get_structured_output(
        self, messages: List[Dict[str, str]], response_model: Any, model: str = "gpt-4o", **kwargs
    ) -> Any:
        max_retries = 3
        for attempt in range(max_retries):
            try:
